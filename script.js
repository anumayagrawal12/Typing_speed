...
tryAgainBtn = document.querySelector(".tnt"),
resetBtn = document.querySelector('.tnr');
timeTag = document.querySelector(".time span b"),
errorTag = document.querySelector(".error span"),
accuTag = document.querySelector(".accu span b");
wpmtag= document.querySelector(".word span");
    ...
    if(charIndex < characters.length - 1 && timeLeft > 0) {
        ...
        if(typedChar == null) {
            ...
            if(characters[charIndex].innerText == typedChar) {
                ...
                error++;
                cpm++;
                let tempValue = inpField.value.split("");
                tempValue.pop();
                tempValue.push(characters[charIndex].innerText);
                inpField.value = tempValue.join("");
...
function initTimer() {
    if(timeLeft > 0) {
        ...
        accuTag.innerText = Math.round(((cpm-error)*100)/cpm);
        clearInterval(timer);
    }
}
...
const tryAgain = () => {
    ...
    clearInterval(timer);
    timeLeft = maxTime;
    charIndex = error = isTyping = cpm=wpm=0;
    wpmtag.innerText= 0;
    accuTag.innerText=0;
    inpField.value = "";
    timeTag.innerText = timeLeft;
...
function resetGame() {
    clearInterval(timer);
    const allLetters = document.querySelectorAll('.typing-text p span');
